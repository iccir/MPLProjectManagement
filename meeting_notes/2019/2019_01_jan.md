...
- Try to pass major ticks to minor tick locators @jklymak
...
- Settled on using logging
    - @jklymak will change #13026 to use logging
...
- @efiring format ticks method takes only values, applies particular formatter
...
- [ ] scipy 2019 talk + tutorial
  - [ ] @WeatherGod is not planning to go to SciPy this year, @story645 has volunteered to teach intro-to-matplotlib tutorial
...
- @anntzer proposal for core devs:
    ...
    - @jklymak needs 100% code coverage
...
### SciPy
- modified Anatomy of Matplotlib by @story645
    - needs assistants
### Students
- maybe @anntzer answering questions
...
### Numfocus Annual Report (@tacaswell wrote it)
...
- Are there projects?
    ...
    - outside matplotlib + mpl related
        - fix viz in geopandas @story645 would mentor, need run by geopandas
        - cartopy - @dopplershift & Elliot have commit bits so stuff could go in
        - factor out pandas plotting into its own package - kinda becomes matplotlib toolkit 
            - @tacaswell maybe, @story645 maybe
...
- @dopplershift can host
- $13k  budget + small grant ($3k from numfocus)
    - 2 days, 5 - 7 travelers?
        - Have to be there: @dopplershift @efiring @tacaswell
        - probably: @story645 + @dorafc
...
- @story645 spoke with geopandas, worried their project is not a full 3 months (but we suspect it is more complicated than they think)
- @tacaswell will send follow up email and include user list.
...
- @tacaswell will send email with announcement + asking for volunteers to help run release.
- get in https://github.com/matplotlib/matplotlib/pull/13030 (ping @WeatherGod)

### Units

- @anntzer - datetime should be pitched to floats & stay there instead of mixed units

...
@dopplershift @tacaswell @efiring @anntzer @dorafc @story645 @jklymak
...
- sort out who should be in the conversation
    ...
    - @timhoffm (?) @dstansby (?) @tacaswell @dorafc @story645 @efiring
...
- @tacaswell  and @story645 will seed initial topics
...
- @efiring : split out  into seperate manual highlights
    - just concatenate whats new as it falls
    - extract a “high-lights” page 
- @jklymak : write the API changes into categories + file for each category:
    - deprecations
    - key word changes
    - removals
    - behavior changes
- Automating?
    - @carreau has a script that can maybe autofind API changes
    - @anntzer might write a script to autofind deprecation changes
...
- https://github.com/matplotlib/matplotlib/pull/11947
    - @jklymak create smaller/unambigious test case that index is all lined up
    - @anntzer plot vector field on top of stream plot
...
- https://github.com/matplotlib/matplotlib/issues/15232 (rcParam expansion)
    - @timhoffm will put in a PR to implement the fix he suggested
...
- https://github.com/matplotlib/matplotlib/pull/14928 - merged
    - needs small followup, @anntzer will edit orginal meeting
- https://github.com/matplotlib/matplotlib/pull/14889
    - merge for arrays 
    - future work: extend for scalermappables
- https://github.com/matplotlib/matplotlib/pull/14130
    - @anntzer proposes pulling these functions out of auto-generate and manually handling them (there are 3 functions with this issue)
...
- discussion about replacing `:math:'gamma``'` with 𝛾 or `\N{GREEK SMALL LETTER GAMMA}`
    - decided against the above, @efiring is very concerned about unicode in docstrings
...
- creating an umbrella package for user contributed colormaps? 
    ...
    - @tacaswell will reach out to colorcet about collaborating on this library
- decison:
    - merge turbo
...
- DivirgingNorm:  https://github.com/matplotlib/matplotlib/pull/15333
    - put center argument in regular Norm?
        - @tacaswell can’t put a center in Norm or it’ll be overconstrained?

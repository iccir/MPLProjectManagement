...
- push back or push off?
    ...
    - @efiring suggests we settle outstanding architecture changes before investing heavily in documentation
...
- Chan Zuckerberg Inititative
    ...
    - half of @tacaswell , 1 full time person
...
@anntzer and @efiring discussed several PRs and issues:
...
- needs second reviewer (@tacaswell)
...
- go to implicit approval model for old PRs)
    ...
    - @tacaswell has been convinced this time around
    - @anntzer will write up PR [done]
...
- @ivanov will release manage
- @tacaswell will send out email announcing this, create new milestone, ask people to start migrating things away from 3.2 or marking as release critical
...
### freetype hacking
@anntzer has a scheme of pre-loading the right version of freetype when running the tests so that the loader does not try to find freetype and hence defers the decision about what version to use until as late as possible.  [now at https://github.com/matplotlib/matplotlib/pull/14874]

- @tacaswell : this seems sketchy, but maybe is OK?
...
- @story645 reached out on twitter for new contirbutors, got 40-50 responses
- @eliasnema is on the call
- @davidevaleriani is interesting in managing blog
...
- get off the ground email Stephany and Elias (cc @tacaswell)
...
- for discourse we want to solve problem of where to run user-user support
    ...
    - SO has it’s issues
    - support via twitter via @story645 is not sustainable
